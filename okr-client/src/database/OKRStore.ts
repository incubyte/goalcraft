...
async function addOkrsDataToDB(objective: {
  objective: string;
...
async function updateOkrsDataToDb(
  objectiveTobeUpdated: ObjectiveType
): Promise<ObjectiveType> {
  let response;
...
async function deleteKeyResultOfObjective(keyResultId: string): Promise<
  KeyResultType & {
    id: string;
    objectiveId: string;
  }
> {
  const response = await fetch(
    `${import.meta.env.VITE_LOCAL_URL}/key-results`,
    {
      method: 'DELETE',
...
type ResponseKeyResultType = KeyResultType & {
  id: string;
  objectiveId: string;
};

async function addKeyResultToObjective(
