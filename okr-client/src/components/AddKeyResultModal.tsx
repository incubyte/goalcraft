...
export default function AddKeyResultModal({
  ...
  const [keyResult, setKeyResult] =
    useState<InsertKeyResultType>(defaultKeyResults);

  function handleAddKeyResult() {
    if (objectives === null) return;

    if (keyResult.title === '' || keyResult.metric === '') {
  ...
  function handleChange(key: string, value: number | string) {
    const updatedKeyResult: InsertKeyResultType = {
      ...keyResult,
      [key]: value,
    };
      ...
      >
        <div className="w-full flex justify-between mb-3">
          <h1 className="text-secondary font-medium mb-2">
            {objectives != null &&
              objectives[keyResultModal.objectiveIndex].objective}
          </h1>
