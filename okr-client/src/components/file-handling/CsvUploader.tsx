...
import { OkrContext } from '../../context/okr-data/okr.provider';
import { getOkrsFromDB, saveCsvDataToDB } from '../../database/okr.store';
import { OkrType, ParsedFileType } from '../../types/okr.types';
        ...
        <div className="p-6 max-w-lg w-full rounded-b-xl">
          <FileUpload
            ...
            onUpload={async event => {
              try {
                const response: ParsedFileType[] = await JSON.parse(event.xhr.response);
                if (response) {
                  await Promise.all(
                    response.map(async parsedFile => {
                      const okrs: OkrType[] = parsedFile.parsedContent;
                      await saveCsvDataToDB(okrs).then(async () => setOkrs(await getOkrsFromDB()));
                    })
                  );
