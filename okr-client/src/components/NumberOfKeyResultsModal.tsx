...
export default function NumberOfKeyResultsModal({
    ...
    function handleGenerateKeyResultFromLLM() {
        if (numberOfKeyResults < 1) {
            ...
            generateKeyResultFromLLM(newObjective, numberOfKeyResults).then((generatedKeyResults: KeyResultType[]) => {
    ...
    return (
        <div className="h-full inset-0 fixed top-0 left-0 right-0 bottom-0 bg-gray-500 bg-opacity-50 flex justify-center items-center z-20">
            <div
                ...
                id="firstKeyResult"
            >
                <CircleX className="w-5 h-5 absolute top-3 right-3 cursor-pointer text-red-500"
                   onClick={() => {
                       setIsGenerate(false);
                   }}/>
                <div className="space-y-2">
                    ...
                    <div className="flex justify-between">
                        ...
                        >
                            <Sparkles
                                className={`w-3 h-3 -rotate-45 ${isGenerating ? "animate-ping" : ""}`}/> Generate
                        </button>
                    </div>
                    <ToastContainer/>
                </div>
